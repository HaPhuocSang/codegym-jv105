...
function elementInspection() {
    ...
    let inspection = parseFloat(document.getElementById("elementValue").value);
    if (isNaN(inspection)) {
        alert("Giá trị nhập vào không hợp lệ!");
        return;
    }
    let result;
    result = NUMBER_INTEGER.includes(inspection)
        ? `${inspection} is in the array.`
        : `${inspection} is not in the array.`;
    document.getElementById("resultLast").textContent = result;
}